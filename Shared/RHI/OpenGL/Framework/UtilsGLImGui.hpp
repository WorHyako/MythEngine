...
static const GLchar* shaderCodeImGuiVertex = R"(
	#version 460 core
...
static const GLchar* shaderCodeImGuiFragment = R"(
	#version 460 core
	in vec2 Frag_UV;
	in	vec4 Frag_Color;
        ...
        for (int n = 0; n < draw_data->CmdListsCount; n++)
        {
            ...
            for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
            {
                ...
                glDrawElementsBaseVertex(GL_TRIANGLES, (GLsizei) pcmd->ElemCount, GL_UNSIGNED_SHORT,
                                         (void*) (intptr_t) (pcmd->IdxOffset * sizeof(ImDrawIdx)), (GLuint) pcmd->VtxOffset);
            }
        }
...
private:
    void defaultInitImGui()
    {
        ...
        cfg.FontDataOwnedByAtlas = false;// manage memory ourselves
        cfg.RasterizerMultiply = 1.5f;   // Brightening up small fonts
        cfg.SizePixels = 768.0f / 32.0f;
        cfg.PixelSnapH = true;// Align every glyph to the pixel boundary
        cfg.OversampleH = 4;
        cfg.OversampleV = 4;
        ImFont* Font = io.Fonts->AddFontFromFileTTF((FilesystemUtilities::GetResourcesDir() + "Fonts/OpenSans-Light.ttf").c_str(), cfg.SizePixels, &cfg);

        unsigned char* pixels = nullptr;
        int width, height;
        io.Fonts->GetTexDataAsRGBA32(&pixels, &width, &height);

        GLuint ioFontTexture;
        glCreateTextures(GL_TEXTURE_2D, 1, &ioFontTexture);
        glTextureParameteri(ioFontTexture, GL_TEXTURE_MAX_LEVEL, 0);
        glTextureParameteri(ioFontTexture, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        glTextureParameteri(ioFontTexture, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        glTextureStorage2D(ioFontTexture, 1, GL_RGBA8, width, height);
        glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
        glTextureSubImage2D(ioFontTexture, 0, 0, 0, width, height, GL_RGBA, GL_UNSIGNED_BYTE, pixels);
        glBindTextures(0, 1, &ioFontTexture);

        io.Fonts->TexID = (ImTextureID) (intptr_t) ioFontTexture;
        io.FontDefault = Font;
...
static void imguiTextureWindowGL(const char* title, uint32_t texId)
{
