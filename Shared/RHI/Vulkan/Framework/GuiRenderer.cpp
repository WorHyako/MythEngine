...
static void addImGuiItem(uint32_t width, uint32_t height, VkCommandBuffer commandBuffer, const ImDrawCmd* pcmd,
	ImVec2 clipOff, ImVec2 clipScale, int idxOffset, int vtxOffset, VkPipelineLayout pipelineLayout)
{
	...
	if (clipRect.x < width && clipRect.y < height && clipRect.z >= 0.0f && clipRect.w >= 0.0f)
	{
		...
		VkRect2D scissor{};
		scissor.offset = { (int32_t)(clipRect.x), (int32_t)(clipRect.y) };
	...
	: Renderer(ctx)
{
	...
	allTextures.push_back(ctx.resources.createFontTexture((FilesystemUtilities::GetResourcesDir() + "Fonts/OpenSans-Light.ttf").c_str()));
	for (auto t : textures)
	...
	dsInfo.textureArrays = { fsTextureArrayAttachment(allTextures) };
	...
	pipelineInfo.useDepth = false;
	pipelineInfo.dynamicScissorState = true;
	const PipelineInfo pInfo = initRenderPass(pipelineInfo, {}, renderPass, ctx.screenRenderPass_NoDepth);

	initPipeline({ (FilesystemUtilities::GetShadersDir() + "Vulkan/VK02_ImGui.vert").c_str(), (FilesystemUtilities::GetShadersDir() + "Vulkan/VK02_ImGui.frag").c_str() }, pInfo, 0, sizeof(uint32_t));
}
