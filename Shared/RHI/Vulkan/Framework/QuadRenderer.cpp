		...
		: Renderer(ctx)
{
	...
	dsInfo.buffers = {storageBufferAttachment(VulkanBuffer{}, 0, vertexBufferSize, VK_SHADER_STAGE_VERTEX_BIT)};
	dsInfo.textureArrays = { fsTextureArrayAttachment(textures) };
	...
	initPipeline({ (FilesystemUtilities::GetShadersDir() + "Vulkan/QuadRenderer/VK_QuadRenderer.vert").c_str(), (FilesystemUtilities::GetShadersDir() + "Vulkan/QuadRenderer/VK_QuadRenderer.frag").c_str() }, pInfo);
}
