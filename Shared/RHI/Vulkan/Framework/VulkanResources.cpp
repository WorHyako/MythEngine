...
#include <RHI/Vulkan/Framework/VulkanResources.hpp>
...
VulkanTexture VulkanResources::addDepthTexture(int texWidth, int texHeight, VkImageLayout layout)
{
    ...
    if(!createImage(vkDev.device, vkDev.physicalDevice, w, h, depthFormat,
...
std::pair<BufferAttachment, BufferAttachment> VulkanResources::makeMeshBuffers(const std::vector<float>& vertices, const std::vector<unsigned>& indices)
{
...
std::pair<BufferAttachment, BufferAttachment> VulkanResources::loadMeshToBuffer(
    const char* filename,
