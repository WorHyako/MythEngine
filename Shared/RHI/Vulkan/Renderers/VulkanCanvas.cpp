	...
	: RendererBase(vkDev, depth)
{
	...
	if (!createColorAndDepthRenderPass(vkDev, (depth.image != VK_NULL_HANDLE), &renderPass_, RenderPassCreateInfo()) ||
		...
		!createGraphicsPipeline(vkDev, renderPass_, pipelineLayout_, { (FilesystemUtilities::GetShadersDir() + "Vulkan/Lines.vert").c_str(), (FilesystemUtilities::GetShadersDir() + "Vulkan/Lines.frag").c_str() }, &graphicsPipeline_, VK_PRIMITIVE_TOPOLOGY_LINE_LIST, (depth.image != VK_NULL_HANDLE), true))
	{
