...
bool ImGuiRenderer::createDescriptorSet(VulkanRenderDevice& vkDev)
{
	...
	for (size_t i = 0; i < vkDev.swapchainImages.size(); i++)
	{
		...
		const VkDescriptorBufferInfo bufferInfo2 = { storageBuffer_[i], 0, ImGuiIdxBufferSize };
...
bool ImGuiRenderer::createMultiDescriptorSet(VulkanRenderDevice& vkDev)
{
	...
	for(size_t i = 0; i < extTextures_.size(); i++)
	{
		textureDescriptors.push_back({
			extTextures_[i].sampler,
			extTextures_[i].image.imageView,
			VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL	/// TODO: select type from VulkanTexture object (GENERAL or SHADER_READ_ONLY_OPTIMAL)
		});
	}
	...
	for (size_t i = 0; i < vkDev.swapchainImages.size(); i++)
	{
		...
		VkWriteDescriptorSet dsImage{};
		dsImage.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
		dsImage.dstSet = descriptorSets_[i];
		dsImage.dstBinding = 3;
		dsImage.dstArrayElement = 0;
		dsImage.descriptorCount = static_cast<uint32_t>(1 + extTextures_.size());
		dsImage.descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
		dsImage.pImageInfo = textureDescriptors.data();
	...
	const std::vector<VulkanTexture>& textures, VkPipelineLayout pipelineLayout)
{
	...
	if(clipRect.x < width && clipRect.y < height && clipRect.z >= 0.0f && clipRect.w >= 0.0f)
	{
		...
		const VkRect2D scissor = {
			{(int32_t)(clipRect.x), (int32_t)(clipRect.y)},
	...
	:RendererBase(vkDev, VulkanImage())
{
	...
	createFontTexture(io, (FilesystemUtilities::GetResourcesDir() + "Fonts/OpenSans-Light.ttf").c_str(), vkDev, font_.image, font_.imageMemory);
	...
	if(!createColorAndDepthRenderPass(vkDev, false, &renderPass_, RenderPassCreateInfo()) ||
		...
		!createGraphicsPipeline(vkDev, renderPass_, pipelineLayout_,
			{(FilesystemUtilities::GetShadersDir() + "Vulkan/VK02_ImGui.vert").c_str(), (FilesystemUtilities::GetShadersDir() + "Vulkan/VK02_ImGui.frag").c_str()},
