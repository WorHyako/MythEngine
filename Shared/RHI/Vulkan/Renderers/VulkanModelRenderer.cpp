...
bool ModelRenderer::createDescriptorSet(VulkanRenderDevice& vkDev, uint32_t uniformDataSize)
{
	...
	for(size_t i = 0; i < vkDev.swapchainImages.size(); i++)
	{
		...
		const VkDescriptorImageInfo imageInfo = { textureSampler_, texture_.imageView, useGeneralTexureLayout_ ? VK_IMAGE_LAYOUT_GENERAL : VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL };
	...
	:RendererBase(vkDev, VulkanImage())
{
	...
	if(!createDepthResources(vkDev, vkDev.framebufferWidth, vkDev.framebufferHeight, depthTexture_) ||
		...
		!createGraphicsPipeline(vkDev, renderPass_, pipelineLayout_, { (FilesystemUtilities::GetShadersDir() + "Vulkan/VK02.vert").c_str(), (FilesystemUtilities::GetShadersDir() + "Vulkan/VK02.frag").c_str() , (FilesystemUtilities::GetShadersDir() + "Vulkan/VK02.geom").c_str() }, &graphicsPipeline_))
	{
...
ModelRenderer::ModelRenderer(
	...
	bool useGeneralTextureLayout,
	VulkanImage externalDepth,
	bool deleteMeshData)
	: useGeneralTexureLayout_(useGeneralTextureLayout)
