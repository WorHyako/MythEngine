...
bool MultiMeshRenderer::createDescriptorSet(VulkanRenderDevice &vkDev)
{
    ...
    for (size_t i = 0; i < vkDev.swapchainImages.size(); i++)
    {
        ...
        const VkDescriptorBufferInfo bufferInfo1 = {storageBuffer_, 0, maxVertexBufferSize_};
        const VkDescriptorBufferInfo bufferInfo2 = {storageBuffer_, maxVertexBufferSize_, maxIndexBufferSize_};
        const VkDescriptorBufferInfo bufferInfo3 = {drawDataBuffers_[i], 0, maxDrawDataSize_};
        const VkDescriptorBufferInfo bufferInfo4 = {materialBuffer_, 0, maxMaterialSize_};
        ...
        const std::array<VkWriteDescriptorSet, 5> descriptorWrites = {
                ...
                bufferWriteDescriptorSet(ds, &bufferInfo1, 1, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER),
                bufferWriteDescriptorSet(ds, &bufferInfo2, 2, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER),
                bufferWriteDescriptorSet(ds, &bufferInfo3, 3, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER),
                bufferWriteDescriptorSet(ds, &bufferInfo4, 4, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER)
        };
...
void MultiMeshRenderer::updateIndirectBuffers(VulkanRenderDevice& vkDev, size_t currentImage, bool* visibility)
{
    ...
    for (uint32_t i = 0; i < maxShapes_; i++)
    {
        ...
        drawCmd.vertexCount = meshData_.meshes_[j].getLODIndicesCount(lod);
        drawCmd.instanceCount = visibility ? (visibility[i] ? 1u : 0u) : 1u;
...
void MultiMeshRenderer::loadDrawData(const char *drawDataFille)
{
    FILE* f = fopen(drawDataFille, "rb");
    ...
    ,RendererBase(vkDev, VulkanImage())    
{
    ...
    const uint32_t offsetAligment = static_cast<uint32_t>(devProps.limits.minStorageBufferOffsetAlignment);
    if ((maxVertexBufferSize_ & (offsetAligment - 1)) != 0)
    {
        int floats = (offsetAligment - (maxVertexBufferSize_ & (offsetAligment - 1))) / sizeof(float);
        for (int ii = 0; ii < floats; ii++)
            meshData_.vertexData_.push_back(0);
        maxVertexBufferSize_ = (maxVertexBufferSize_ + offsetAligment) & ~(offsetAligment - 1);
    }
