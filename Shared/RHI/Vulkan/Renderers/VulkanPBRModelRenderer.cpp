	...
	: RendererBase(vkDev, VulkanImage())
{
	...
	gli::texture gliTex = gli::load_ktx((FilesystemUtilities::GetResourcesDir() + "Data/brdfLUT.ktx").c_str());
	gli::tvec3<GLsizei> extent(gliTex.extent(0));
	...
	if(!createColorAndDepthRenderPass(vkDev, true, &renderPass_, RenderPassCreateInfo()) ||
		...
		!createGraphicsPipeline(vkDev, renderPass_, pipelineLayout_,
			{(FilesystemUtilities::GetShadersDir() + "Vulkan/VK_GLTF_PBRShader.vert").c_str(), (FilesystemUtilities::GetShadersDir() + "Vulkan/VK_GLTF_PBRShader.frag").c_str() },
			&graphicsPipeline_))
	{
